...
get_header(); ?>
		...
		<section id="primary">
			...
			if ( $query->have_posts() ) : ?>
				...
				<?php 
					/* ESTILO DIFERENTE PARA TRES PRIMEROS POSTS */
					if ($i==1 && !is_paged()):
				?>

				<article id="post-<?php the_ID(); ?>" <?php post_class('featured reportajes-topfeatured reportajes-featured'); ?>>
					<a href="<?php the_permalink(); ?>" title="Leer <?php the_title('&#8220;','&#8221;')?>" rel="bookmark" class="reportajes-topfeatured-link">
					</a>
				...
				<article id="post-<?php the_ID(); ?>" <?php post_class('featured featured-secondary reportajes-featured reportajes-secondary '.$reportajealign); ?>>
					<header class="entry-header">
						<?php if ( 'post' == get_post_type() ) : ?>
							<div class="entry-meta">
								<?php toolbox_posted_on(); ?>
					...
					<div class="entry-summary">
						<a href="<?php the_permalink(); ?>" title="Leer <?php the_title('&#8220;','&#8221;')?>" rel="bookmark">
							<?php the_post_thumbnail('reportajes',array('class' => 'reportajes-mini')); ?>
						</a>
